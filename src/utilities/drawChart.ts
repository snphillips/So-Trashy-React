...
export function drawChart(data: DataItemType[], refuseType: RefuseTypes, year: number) {
  ...
  const getPopulation = (d: DataItemType) => (year >= 2020 ? d._2020_population : d._2010_population);
  ...
  if (xAxisBottomGroup.empty()) {
    xAxisBottomGroup = g.append('g').attr('class', 'x-axis-bottom').attr('transform', `translate(0, ${innerHeight})`);
  }
  ...
  function handleMouseOver(this: SVGRectElement, event: MouseEvent | KeyboardEvent, d: DataItemType) {
    ...
    if (event instanceof MouseEvent) {
      tooltip.style('left', `${event.pageX + 15}px`).style('top', `${event.pageY - 120}px`);
    } else {
  ...
  function generateTooltipHTML(d: DataItemType, year: number): string {
    ...
    const listItems = refuseCategories
      .map((category) => {
        const percent = totalRefuse ? ((d[category.key] as number) * 100) / totalRefuse : 0;
